      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <div ref={langRef} className="relative">
            ...
            {langOpen && (
              <div className="absolute right-0 mt-2 w-64 max-h-72 overflow-y-auto rounded-xl border border-border bg-bg-card shadow-lg py-1 z-50 grid grid-cols-2 gap-0">
                {LANGS.map((l) => (
                  <button
                    key={l.code}
                    onClick={() => switchLang(l.code)}
                    className={`text-left px-3 py-1.5 text-xs transition-colors hover:bg-accent/10 truncate ${
